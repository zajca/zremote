# RFC: OpenViking Integration for MyRemote
...
MyRemote manages remote machines with terminal sessions and agentic loop monitoring, but lacks persistent knowledge about the projects running on those machines. Each time an agentic loop starts, it has zero context about prior work, common patterns, or accumulated insights for that project.
...
New file: `crates/myremote-protocol/src/knowledge.rs`
...
New module: `crates/myremote-agent/src/knowledge/`
...
```rust
/// Generate ov.conf from MyRemote config values.
...
New file: `crates/myremote-server/migrations/005_knowledge.sql`
...
New file: `crates/myremote-server/src/routes/knowledge.rs`
...
```markdown
# Project Knowledge (auto-generated by MyRemote)
...
| `OPENVIKING_DATA_DIR` | No | Agent | Data directory for OV (default: `~/.myremote/openviking/`) |
...
- [ ] **Protocol types**
  - [ ] Create `crates/myremote-protocol/src/knowledge.rs` with all types
...
- [ ] **Protocol integration**
  ...
  - [ ] Run `cargo test -p myremote-protocol` - all pass
  - [ ] Run `cargo clippy -p myremote-protocol` - no warnings

- [ ] **Agent knowledge module**
  - [ ] Create `crates/myremote-agent/src/knowledge/mod.rs` - `KnowledgeManager`
  - [ ] Create `crates/myremote-agent/src/knowledge/process.rs` - `OvProcess` (spawn, health check, graceful shutdown)
  - [ ] Create `crates/myremote-agent/src/knowledge/client.rs` - `OvClient` (reqwest HTTP client)
  - [ ] Create `crates/myremote-agent/src/knowledge/config.rs` - `generate_ov_conf()`, `model_prefix()`
...
- [ ] **Agent integration**
  ...
  - [ ] Run `cargo test -p myremote-agent` - all pass
  - [ ] Run `cargo clippy -p myremote-agent` - no warnings

### Phase 2: Server-side + Indexing

- [ ] **Database migration**
  - [ ] Create `crates/myremote-server/migrations/005_knowledge.sql`
...
- [ ] **Server routes**
  - [ ] Create `crates/myremote-server/src/routes/knowledge.rs`
...
- [ ] **Auto-indexing**
  ...
  - [ ] Run `cargo test -p myremote-server` - all pass
  - [ ] Run `cargo clippy -p myremote-server` - no warnings
...
|---|---|---|
| Protocol types | Serde roundtrip, enum serialization | `crates/myremote-protocol/src/knowledge.rs` |
| Config generation | ov.conf output for all providers | `crates/myremote-agent/src/knowledge/config.rs` |
| Model prefix mapping | All provider -> prefix mappings | `crates/myremote-agent/src/knowledge/config.rs` |
| Memory dedup logic | Insert/update/skip behavior | `crates/myremote-server/src/routes/knowledge.rs` |
