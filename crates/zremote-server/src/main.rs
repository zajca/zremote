...
async fn main() {
    ...
    // Fail fast if ZREMOTE_TOKEN is not set
    let agent_token = std::env::var("ZREMOTE_TOKEN").unwrap_or_else(|_| {
        tracing::error!("ZREMOTE_TOKEN environment variable is required");
        std::process::exit(1);
    });

    let database_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        tracing::warn!("DATABASE_URL not set, using default: sqlite:zremote.db");
        "sqlite:zremote.db".to_string()
    });
...
mod tests {
    ...
    async fn root_returns_server_name() {
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        assert_eq!(&body[..], b"ZRemote Server");
    }
