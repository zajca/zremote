mod format;
mod input;
mod socket;

use std::io::Read;
use std::process::Command;

/// Get the current git branch name from the working directory.
/// Returns `None` if not in a git repo or command fails.
fn git_branch(cwd: &str) -> Option<String> {
    let output = Command::new("git")
        .args(["rev-parse", "--abbrev-ref", "HEAD"])
        .current_dir(cwd)
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::null())
        .output()
        .ok()?;

    if output.status.success() {
        let branch = String::from_utf8_lossy(&output.stdout).trim().to_string();
        if branch.is_empty() {
            None
        } else {
            Some(branch)
        }
    } else {
        None
    }
}

fn main() {
    // Read all of stdin
    let mut raw = String::new();
    if std::io::stdin().read_to_string(&mut raw).is_err() {
        return;
    }

    // Parse JSON (silently produce empty output on parse failure)
    let input: input::StatusInput = match serde_json::from_str(&raw) {
        Ok(v) => v,
        Err(_) => return,
    };

    // Get git branch from cwd
    let branch = input.cwd.as_deref().and_then(git_branch);

    // Format and print status line
    let status = format::format_status(&input, branch.as_deref());
    if !status.is_empty() {
        println!("{status}");
    }

    // Forward raw JSON to ZRemote agent via Unix socket
    socket::send_to_agent(raw.as_bytes());
}
