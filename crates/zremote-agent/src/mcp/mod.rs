...
fn resource_list(_server: &KnowledgeMcpServer) -> Vec<serde_json::Value> {
    ...
    for cat in &categories {
        resources.push(serde_json::json!({
            "uri": format!("zremote://memories/{cat}"),
...
async fn read_resource(server: &KnowledgeMcpServer, uri: &str) -> serde_json::Value {
    if uri == "zremote://context" {
        ...
        let content = if let Ok(data) = tokio::fs::read_to_string(&claude_md_path).await {
            let marker = "<!-- ZRemote Knowledge (auto-generated, do not edit below) -->";
    ...
    } else if let Some(category) = uri.strip_prefix("zremote://memories/") {
...
mod tests {
    ...
    fn resource_list_has_context_and_categories() {
        ...
        assert_eq!(resources[0]["uri"], "zremote://context");
    }
    ...
    async fn handle_resources_list() {
        ...
        let uris: Vec<&str> = resources
            ...
            .collect();
        assert!(uris.contains(&"zremote://memories/pattern"));
        assert!(uris.contains(&"zremote://memories/decision"));
        assert!(uris.contains(&"zremote://memories/pitfall"));
        assert!(uris.contains(&"zremote://memories/architecture"));
        assert!(uris.contains(&"zremote://memories/convention"));
    }
    ...
    async fn handle_resources_read_context_no_file() {
        ...
        let msg = r#"{"jsonrpc":"2.0","id":6,"method":"resources/read","params":{"uri":"zremote://context"}}"#;
        let resp = handle_jsonrpc_message(&server, msg).await.unwrap();
        let contents = resp["result"]["contents"].as_array().unwrap();
        assert_eq!(contents.len(), 1);
        assert_eq!(contents[0]["uri"], "zremote://context");
    ...
    async fn handle_resources_read_context_with_marker() {
        ...
        let server = KnowledgeMcpServer::new(dir.path().to_path_buf(), 8741);
        let msg = r#"{"jsonrpc":"2.0","id":7,"method":"resources/read","params":{"uri":"zremote://context"}}"#;
        let resp = handle_jsonrpc_message(&server, msg).await.unwrap();
    ...
    async fn handle_resources_read_unknown_uri() {
        let server = KnowledgeMcpServer::new(PathBuf::from("/tmp/test"), 8741);
        let msg = r#"{"jsonrpc":"2.0","id":8,"method":"resources/read","params":{"uri":"zremote://unknown"}}"#;
        let resp = handle_jsonrpc_message(&server, msg).await.unwrap();
    ...
    async fn handle_resources_read_memories_category() {
        let dir = tempfile::tempdir().unwrap();
        let server = KnowledgeMcpServer::new(dir.path().to_path_buf(), 8741);
        let msg = r#"{"jsonrpc":"2.0","id":11,"method":"resources/read","params":{"uri":"zremote://memories/pattern"}}"#;
        let resp = handle_jsonrpc_message(&server, msg).await.unwrap();
        let text = resp["result"]["contents"][0]["text"].as_str().unwrap();
    ...
    async fn handle_resources_read_multiple_categories() {
        ...
        for category in &["decision", "pitfall", "architecture", "convention"] {
            let msg = format!(
                r#"{{"jsonrpc":"2.0","id":20,"method":"resources/read","params":{{"uri":"zremote://memories/{category}"}}}}"#
            );
            let resp = handle_jsonrpc_message(&server, &msg).await.unwrap();
            let text = resp["result"]["contents"][0]["text"].as_str().unwrap();
    ...
    async fn handle_context_with_claude_md_no_marker() {
        ...
        let server = KnowledgeMcpServer::new(dir.path().to_path_buf(), 8741);
        let msg = r#"{"jsonrpc":"2.0","id":21,"method":"resources/read","params":{"uri":"zremote://context"}}"#;
        let resp = handle_jsonrpc_message(&server, msg).await.unwrap();
        let text = resp["result"]["contents"][0]["text"].as_str().unwrap();
